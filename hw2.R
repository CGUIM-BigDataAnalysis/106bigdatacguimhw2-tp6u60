...
studentc103104<-studentc103104%>% #103104年度學生出國目的地和總數
...
students103104<-students103104%>%  #103104年度出國交流學校的學生總數
  mutate("alltotal"=rowSums(students103104[2:3],na.rm = FALSE,dims = 1))%>%
  arrange(desc(alltotal))
head(studentc103104,1)
head(studentc103104,10)
head(students103104,1)
head(students103104,10)
...
peopleamountcount105<-peopleamountcount105%>%
  ...
  arrange(desc(amount))
head(peopleamountcount105,1)
head(peopleamountcount105,10)
...
jointwstugoout<-jointwstugoout%>%
  ...
  group_by(region)%>%
  summarise(value=sum(value))
drawjointwstygoout<-county_choropleth(jointwstugoout,title = "台灣學生去各國家留學人數")
ggplotly(drawjointwstygoout)
